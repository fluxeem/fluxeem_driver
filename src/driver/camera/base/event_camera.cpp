...
namespace fluxeem
{
    ...
    bool EventCamera::start() {
        ...
        if (internal_event_callback_id_ == std::numeric_limits<uint32_t>::max()) {
            internal_event_callback_id_ = pipeline_->registerEventCallback([this](EventIterator_t begin, EventIterator_t end) {
                eventStreamArrived(begin, end);
            });
        }

        // Register raw buffer callback for file recording
        if (internal_raw_buffer_callback_id_ == std::numeric_limits<uint32_t>::max()) {
            internal_raw_buffer_callback_id_ = pipeline_->registerRawBufferCallback([this](const std::vector<uint8_t>& buffer) {
                if (is_recording_) {
    ...
    bool EventCamera::close()
    {
        ...
        try
        {
            ...
            tools_.clear();
            internal_event_callback_id_ = std::numeric_limits<uint32_t>::max();
            internal_raw_buffer_callback_id_ = std::numeric_limits<uint32_t>::max();
            pipeline_.reset();
            sensor_.reset();
