...
namespace fluxeem
{
	...
	inline int EVT3Decoder::handleTimeLow(const Evt3Raw::RawEvent *cur_raw_ev)
	{
		...
		// Handle custom trigger (polarity 2 indicates software trigger)
		if (custom_trigger_in_by_ts_ != 0 && last_timestamp_.time >= custom_trigger_in_by_ts_)
		{
			if (ev_trigger_in_handling_callback_)
			{
				ev_trigger_in_handling_callback_(EventTriggerIn(0, 2, last_timestamp_.time));
			}
			custom_trigger_in_by_ts_ = 0;
		}
	...
	void EVT3Decoder::setCustomTriggerIn(Timestamp ts)
	{
		custom_trigger_in_by_ts_ = ts;
	}
