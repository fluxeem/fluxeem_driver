...
namespace fluxeem
{
	...
	public:
		...
		/// @brief Suppress the next N timestamp jump/rollback warnings after a seek.
		/// When count > 0, the first N anomalies detected in validateAndUpdateTimestamp
		/// are silently ignored instead of printed. This prevents noisy logs during
		/// file seek operations while still reporting genuine errors.
		/// @param count Number of warnings to suppress (default 1)
		void suppressNextTsJumpWarning(uint32_t count = 1) override;
	...
	private:
		...
		// Seek discontinuity suppression: counts how many timestamp anomaly warnings
		// should still be silently skipped (decremented each time one fires).
		std::atomic<uint32_t> ts_jump_suppress_count_{0};
	};
